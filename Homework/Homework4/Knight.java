package Homework.Homework4;
import java.util.List;
import java.util.Random;

public class Knight extends BaseHero {
    
    private static final String HERO_KNIGHT = "Knight #%d";

    /**
     * это копейщик
     */
    public Knight(List<BaseHero> side) {
        super(side);
        super.setName(String.format(HERO_KNIGHT, ++ Knight.number));
        super.setAttack(4);
        super.setDefence(5);
        super.setInitShoot(0);
        super.setShoot(this.initShoot);
        super.setDamage(new Random().nextInt(1, 3));
        super.setInitHealth(10);
        super.setHealth(this.initHealth);
        super.setSpeed(4);
        super.setDelivery(false);
        super.setMagic(false);
        
    }

    /**
     * Дополнительный конструктор, на случай, если пользователь будет вводить имя
     * @param name введенное имя
     */
    public Knight (String name) {
        this.name = name; 
    }

// проверяет, нет ли кого впереди него, если да - атакует, если нет - делает шаг на 1 клетку
    @Override
    public void step(List<BaseHero> target, List<BaseHero> ours) {
               
        //if target.pos.y == this.pos.y+1  _> target.health = target.health - super.getDamage();
        //else this.pos.y = this.pos.y+1
    }
}

    

